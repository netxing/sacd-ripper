 ...
 * but WITHOUT ANY WARRANTY; without even thenum audio_packet_data_type_enum {
      DATA_TYPE_AUDIO           = 2
    , DATA_TYPE_SUPPLEMENTARY   = 3
    , DATA_TYPE_PADDING        = 7
};

typedef enum audio_packet_data_type_enum audio_packet_data_type_t;ed warranty of
...
typedef enum character_set_enum character_set_t;
...
typedef enum genre_enum genre_t;
...
typedef enum category_enum category_t;

extern const char *album_category[];

enum track_type_enum {
...
typedef enum track_type_enum track_type_t;
