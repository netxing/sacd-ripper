...
int scarletbook_process_frames(scarletbook_handle_t *handle, uint8_t *read_buffer, int blocks_read_in, int last_block, frame_read_callback_t frame_read_callback, void *userdata)
{
    ...
    while (blocks_read--)
    {
        ...
        if (handle->frame_info_idx == handle->audio_sector.header.packet_info_count) 
        {
            handle->frame_info_idx = 0;
        ...
        while (handle->frame_info_idx < handle->audio_sector.header.packet_info_count && sector_bad_reads == 0)
        {
            audio_packet_info_t* packet = &handle->audio_sector.packet[handle->frame_info_idx];
